...
public class SecurityManager {
	...
	private String getFormatData(int uid) {
		...
		return StringEncode.hexEncode(temp);
	}
	...
	public String encryData(String plainData) throws Exception {
		...
		try {
			String p = StringEncode.hexEncode(plainData.getBytes(ENCODE_TYPE));
	...
	public String decryData(String data) throws Exception {
		...
		try {
			...
			return new String(StringEncode.hexDecode(s), ENCODE_TYPE);
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 */
	public static String encryMD5(String varl) throws NoSuchAlgorithmException {
		 if (varl == null || varl.trim().length() < 1) {
	            return null;
	        }
	        try {
	            return getMD5(varl.getBytes("UTF-8"));
